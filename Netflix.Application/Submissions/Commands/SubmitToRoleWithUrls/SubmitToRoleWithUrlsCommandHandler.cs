...
{
    internal class SubmitToRoleWithUrlsCommandHandler : IRequestHandler<SubmitToRoleWithUrlsCommand, Submission>
    {
        ...
        public SubmitToRoleWithUrlsCommandHandler(ISubmissionRepository submissionRepository, IClientRepository clientRepository, ICastingCallRepository castingCallRepository, ICloudStorageService cloudStorageService)
        {
        ...
        public async Task<Submission> Handle(SubmitToRoleWithUrlsCommand request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                // 2. Add media URIs to the submission
            ...
            {
                // Clean up Cloudinary files so they don't become orphans
                var deleteTasks = request.MediaUrls
                    .Select(url => _cloudStorageService.RemoveBlobByUrlAsync(url));


                await Task.WhenAll(deleteTasks.Select(async t =>
                {
                    try { await t; }
                    catch { /*should log*/ }
                }));

                throw; // re-throw original exception to the controller
            }
        }
    }
}
